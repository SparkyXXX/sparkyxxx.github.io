...
终端（Terminal）：最早是指用于与计算机交互的物理设备（如键盘和显示器）。用户可以通过终端输入命令，并查看计算机返回的输出。在现代计算机中，终端通常指的是一个程序或界面，它提供了一个命令行界面（CLI），允许用户与操作系统或其他程序进行交互。

终端仿真器（Terminal Emulator）：是一种软件应用程序，它模拟传统终端的功能，使用户能够在图形用户界面（GUI）环境中使用命令行界面。终端仿真器允许用户在桌面环境中运行命令行程序，处理输入和输出，并显示文本信息。

GNOME Terminal：ubuntu 是基于 GNOME 桌面环境的 Linux 发行版，系统自带的终端是 GNOME Terminal，它提供了一个用户友好的界面，支持多标签页、快捷键、可定制的外观和行为等功能。用户可以在 GNOME Terminal 中输入 shell 命令，运行程序，以及进行其他与命令行相关的操作。

使用 ctrl + shift + v 进行粘贴时出现`[[200~~` ，是因为终端仿真器误解了剪贴板中的内容，或复制的内容中含有非标准字符，仿真器将其解释为了控制序列。

Shell 环境是指用户与操作系统进行交互的命令行界面或解释器。它是一个用户与操作系统核心（内核）之间的桥梁，用户可以通过 Shell 输入命令，Shell 将这些命令翻译并传递给操作系统执行，然后将结果返回给用户。不同类型的shell包括：

- Bash（Bourne Again Shell）：这是 Linux 系统中最常用的 Shell，功能强大且兼容性好。
- sh（Bourne Shell）：最早期的 UNIX Shell 之一，Bash 是它的增强版。
- csh（C Shell）：C 语言风格的 Shell，语法与 C 语言类似。
- ksh（Korn Shell）：结合了 Bourne Shell 和 C Shell 的优点，功能较为丰富。
- zsh（Z Shell）：功能非常强大且高度可定制化，许多高级用户喜欢使用。
...
——call——方法：括号加参数，不用.方法+括号和方法的方便调用方式

修复dark_mode下出现light_mode加载动画的bug



np_array的图片是HWC（height，width，channel），用shape属性可以查看，cv.imread得到的是ndarray
...
transforms：对图片进行变换，Compose组合（传入transform列表）、ToTensor转为张量、Normalize归一化、Resize尺寸变化、CenterCrop中心裁剪。transforms相当于一个工具箱，使用之前需要先通过里面的类创建自己的工具，然后再使用这个工具

ToTensor将PILImage或numpy.ndarray转化为Tensor

pycharm按ctrl+p弹出参数提示
