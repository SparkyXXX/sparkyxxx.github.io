...
加载数据：pytorch中的数据主要是两个类型，一个是dataset，一个是dataloader。dataset提供一种获取数据及其labl并进行编号的方式，dataloader用于打包数据成batch并为后面的网络提供不同的数据形式。dataset实现的功能包括如何获取每一个数据及其label、提供数据总数。

self相当于类当中的一个全局变量，类的使用
