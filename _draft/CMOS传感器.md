![v2-4b4efd32460417bf47634f59f57c890c_1440w](../assets/post-pics/v2-4b4efd32460417bf47634f59f57c890c_1440w.webp)
...
在按下快门的一刻，外界光线(Photon Flux)会穿过镜片组(External Lens + Micro Lens)聚集；经过 color filter 滤波后打到感光元件上，其内部将光信号转为像素阵列的形式，并经 ADC 转成数字信号，最后交由图像处理器(ISP，Image Signal Processor)处理成一张数码照片。

![image-20250526215913623](../assets/post-pics/image-20250526215913623.png)

数字图像形成过程

# 二、工作原理

CIS 的电路由像敏单元阵列、行驱动器、列驱动器、时序控制逻辑、增益控制器(PGA，可编程增益放大器)、AD 转换器、数据总线输出接口、控制接口几部分集成在同一块硅片构成。有的芯片还集成了其它数字信号处理电路以进行后续的 ISP 处理。像敏元阵列和像元结构(以 PPS 为例)如图。
...
像敏元阵列结构：1 一垂直移位寄存器：2 一水平移位寄存器；3 一水平扫描开关；4 一垂直扫描开关；5 一像敏元阵列；6 一信号线；7 一像敏元
...
主要分为 FSI(前照式)和 BSI(背照式)两种。在 BS 基础上的改良还有 Stack(堆栈式)和 Quad Bayer 阵列(四合一像素技术)。
...
![image-20250526220341531](../assets/post-pics/image-20250526220341531.png)
...
MIPI 和 DVP 接口比较https://blog.csdn.net/quicmous/article/details/114922463
