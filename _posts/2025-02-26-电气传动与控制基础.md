...
直流他励电动机启动时，需要先建立磁场，再加电枢电压。如果先施加电枢电压，由于电机硅钢片中的铁损存在剩磁（即$$\Phi$$为一个很小的值），一旦施加电压，$$n = \frac{U}{C_e\Phi} - \frac{R}{C_e C_m \Phi^2}T_e$$中的第一项会非常大，对电机造成不可逆的损坏。启动方式有降压启动后逐渐升到额定电压、电枢串电阻启动后分级切除（目前已被新型器件），目的都是缓冲启动时的冲击。另外，需要保持最大启动电枢电流启动。（==这是为什么==）
...
比例-积分调节器的输出：$$U_{ct} = K_p\Delta U_n + \frac{1}{\tau_1}\int_0^t \Delta U_n \text{d}t$$，开始时比例环节起主导作用，实现快速控制，接近稳态时积分环节起主导作用.

==输出电压取决于$$\Delta U_n$$的历史值，还有别的嘛？==

![image-20250311004950356](../assets/post-pics/image-20250311004950356.png)
...
抗扰动过程分析：PI 控制综合了比例控制和积分控制两种规律的优点，又克服了各自的缺点。比例部分能迅速响应控制作用，积分部分则最终消除稳态偏差。PI 调节器构成的五警察调速系统，解决了静态精度和动态稳定性以及快速性之间的矛盾（波形图略）
...
小结：单闭环有静差调速系统可使得系统静特性变硬，在静差率要求一定的前提下调速范围变宽，且系统具有良好的抗扰性能，但存在两个问题：一是静态精度和动态稳定性之间存在上述的矛盾，二是启动电流过大，对速度闭环并不能改善电流冲击，因为电流的变化反映到转速上时需要时间的，该系统且没有四象限运行能力。
...
- 电压负反馈调速系统：不带测速发电机，而是在电机上并联较大的采样电阻，以该电阻的电压值作为转速的近似反馈
  ...
  注意$$R_a$$环节不在反馈包围内，对应的项不用除以$$(1+K)$$；另外，由于左侧控制器为弱电，右侧电机为强电，反馈回的电压需要经变压器或光耦等方式进行强弱电的隔离。
...
![image-20250310231702508](../assets/post-pics/image-20250310231702508.png)
...
$$
\begin{aligned}
&W(s)=\frac{K}{(T_s s + 1)(T_m T_l s^2 + T_m s + 1)} \quad \text{式中}K = K_p K_s \alpha / C_e \\
&W(s)=\frac{K}{s(T_s s + 1)(T_m T_l s^2 + T_m s + 1)} \quad \text{式中 }K = K_s \alpha / \tau C_e \\
&W(s)=\frac{K(K_p \tau s + 1)}{s(T_s s + 1)(T_m T_l s^2 + T_m s + 1)}  \quad \text{式中 }K = K_s \alpha / \tau C_e
\end{aligned}
$$
...
对单位阶跃输入$$U_n^*(s) = \frac{U_n^*}{s}$$的稳态误差：
...
$$
\begin{aligned}
&\Delta U_n(s)=I_{dL}(s)\cdot\frac{\frac{R(T_l s + 1)\alpha}{C_e(T_m T_l s^2 + T_m s + 1)}}{1 + \frac{K}{(T_s s + 1)(T_m T_l s^2 + T_m s + 1)}}\\
&\Delta U_n(s)=I_{dL}(s)\cdot\frac{\frac{R(T_l s + 1)\alpha}{C_e(T_m T_l s^2 + T_m s + 1)}}{1 + \frac{K}{s(T_s s + 1)(T_m T_l s^2 + T_m s + 1)}}\\
&\Delta U_n(s)=I_{dL}(s)\cdot\frac{\frac{R(T_l s + 1)\alpha}{C_e(T_m T_l s^2 + T_m s + 1)}}{1 + \frac{K(K_p \tau s + 1)}{s(T_s s + 1)(T_m T_l s^2 + T_m s + 1)}}
\end{aligned}
$$

对单位阶跃扰动$$I_{dL}(s) = \frac{I_{dL}}{s}$$引起的稳态误差：

$$
\begin{aligned}
&\Delta U_n=\lim_{s \to 0} s\cdot\Delta U_n(s)=\lim_{s \to 0} s\cdot\frac{I_{dL}}{s}\cdot\frac{\frac{R(T_l s + 1)\alpha}{C_e(T_m T_l s^2 + T_m s + 1)}}{1 + \frac{K}{(T_s s + 1)(T_m T_l s^2 + T_m s + 1)}}=\frac{R I_{dL}\alpha}{C_e(1 + K)}\\
&\Delta U_n=\lim_{s \to 0} s\cdot\Delta U_n(s)=\lim_{s \to 0} s\cdot\frac{I_{dL}}{s}\cdot\frac{\frac{R(T_l s + 1)}{C_e(T_m T_l s^2 + T_m s + 1)}}{1 + \frac{K}{s(T_s s + 1)(T_m T_l s^2 + T_m s + 1)}} = 0\\
&\Delta U_n=\lim_{s \to 0} s\cdot\Delta U_n(s)=\lim_{s \to 0} s\cdot\frac{I_{dL}}{s}\cdot\frac{\frac{R(T_l s + 1)}{C_e(T_m T_l s^2 + T_m s + 1)}}{1 + \frac{K(K_p \tau s + 1)}{s(T_s s + 1)(T_m T_l s^2 + T_m s + 1)}} = 0
\end{aligned}
$$
...
使用电流互感器 TA 经过二极管不控整流桥获得电流反馈，另外，使用稳压管模拟以下特性

==稳压管是怎么做到引入电流截止负反馈的==

![image-20250311001634354](../assets/post-pics/image-20250311001634354.png)

令电流反馈系数$$\beta_i = \frac{U_i}{I_d}$$，临界截止电流$$I_{dcr} = \frac{U_{VS}}{\beta_i}$$
