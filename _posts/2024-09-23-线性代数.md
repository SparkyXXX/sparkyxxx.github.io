...
向量之间有线性相关和线性无关的概念。若一组向量$$\{\mathbf{v_\text{1}}, \mathbf{v_\text{2}}, \ldots, \mathbf{v_\text{n}}\}$$的线性组合等于零向量的唯一解，是所有系数均为零时，称这个向量组是线性无关的；否则称为线性相关。

$$
c_\text{1} \mathbf{v_\text{1}} + c_\text{2} \mathbf{v_\text{2}} + \cdots + c_\text{n} \mathbf{v_\text{n}} = \mathbf{0}
$$
...
$$
\mathbf{A} = \begin{pmatrix} a_\text{11} & a_\text{12} \\ a_\text{21} & a_\text{22} \end{pmatrix}, \quad \mathbf{B} = \begin{pmatrix} b_\text{11} & b_\text{12} \\ b_\text{21} & b_\text{22} \end{pmatrix}
$$

$$
\mathbf{C} = \mathbf{A} + \mathbf{B} = \begin{pmatrix} a_\text{11} + b_\text{11} & a_\text{12} + b_\text{12} \\ a_\text{21} + b_\text{21} & a_\text{22} + b_\text{22} \end{pmatrix} \\ c\mathbf{A} = \begin{pmatrix} ca_\text{11} & ca_\text{12} \\ ca_\text{21} & ca_\text{22} \end{pmatrix}
$$

#### 乘法

$$
\mathbf{A} = \begin{pmatrix} a_\text{11} & a_\text{12} & \cdots & a_\text{1n} \\ a_\text{21} & a_\text{22} & \cdots & a_\text{2n} \\ \vdots & \vdots & \ddots & \vdots \\ a_\text{m1} & a_\text{m2} & \cdots & a_\text{mn} \end{pmatrix}_{m \times n}, \quad \mathbf{B} = \begin{pmatrix} b_\text{11} & b_\text{12} & \cdots & b_\text{1k} \\ b_\text{21} & b_\text{22} & \cdots & b_\text{2k} \\ \vdots & \vdots & \ddots & \vdots \\ b_\text{n1} & b_\text{n2} & \cdots & b_\text{nk} \end{pmatrix}_{n \times k}
$$

$$
\mathbf{C} = \mathbf{A} \mathbf{B} = \begin{pmatrix} c_\text{11} & c_\text{12} & \cdots & c_\text{1k} \\ c_\text{21} & c_\text{22} & \cdots & c_\text{2k} \\ \vdots & \vdots & \ddots & \vdots \\ c_\text{m1} & c_\text{m2} & \cdots & c_\text{mk} \end{pmatrix}_{m \times k}
$$

其中$$C$$的第$$i$$行第$$j$$列的元素为$$A$$中第$$i$$行和$$B$$的第$$j$$列作内积得到

$$
c_\text{ij} = a_\text{i1}b_\text{1j} + a_\text{i2}b_\text{2j} + \cdots + a_\text{in}b\_\text{nj}
$$
...
$$
\mathbf{A} = \begin{pmatrix}a_\text{11} & a_\text{12} & \cdots & a_\text{1n} \\a_\text{21} & a_\text{22} & \cdots & a_\text{2n} \\\vdots & \vdots & \ddots & \vdots \\a_\text{m1} & a_\text{m2} & \cdots & a\_\text{mn}\end{pmatrix}
$$

$$
\mathbf{A^T} = \begin{pmatrix}a_\text{11} & a_\text{21} & \cdots & a_\text{m1} \\a_\text{12} & a_\text{22} & \cdots & a_\text{m2} \\\vdots & \vdots & \ddots & \vdots \\a_\text{1n} & a_\text{2n} & \cdots & a\_\text{mn}\end{pmatrix}
$$
...
$$\mathbf{A}_{n\times n}$$为可逆矩阵，若存在矩阵$$\mathbf{B}$$使得$$\mathbf{A} \mathbf{B} = \mathbf{B} \mathbf{A} = \mathbf{I}_\text{n}$$ ，则称$$\mathbf{B}$$为矩阵$$\mathbf{A}$$的逆，记为$$\mathbf{A}^{-1}$$
...
- 单位矩阵：对角元全为 1，非对角元全为 0，记为$$I_\text{n}$$
- 对角矩阵：非对角元全为 0
...
- 某一行/列乘$$k$$，行列式变为$$k$$倍；$$ \vert \mathbf{kA} \vert = k^n \vert \mathbf{A} \vert $$
- 行列互换，正负号改变
- 行列倍加，行列式不变
...
$$
\text{det}(\mathbf{A}) = \begin{vmatrix} a_\text{11} & a_\text{12} \\ a_\text{21} & a_\text{22} \end{vmatrix} = a_\text{11}a_\text{22} - a_\text{12}a_\text{21}
$$

$$n$$阶行列式的一般定义涉及逆序数和全排列，较为复杂

- 逆序数：在一个排列中，对于两个元素$$a_\text{i}$$和$$a_\text{j}$$，如果$$i < j$$且$$a_\text{i} > a_\text{j}$$，则称这对元素为一个逆序对。逆序数就是逆序对的数量，可描述一个排列的“混乱程度”。正式定义为

  给定一个长度为$$n$$的排列$$a_\text{1}, a_\text{2}, \ldots, a_\text{n}$$其逆序数$$I$$定义如下，其中$$\mathbb{I}(P)$$是指示函数，当 $$P$$为真时取值 1，否则取值 0

$$
I = \sum\_{1 \leqslant i < j \leqslant n} \mathbb{I}(a_\text{i} > a_\text{j})
$$

给定$$\mathbf{A}_{n\times n}$$，其行列式为

$$
\mathbf{A} = \begin{pmatrix} a_\text{11} & a_\text{12} & \cdots & a_\text{1n} \\ a_\text{21} & a_\text{22} & \cdots & a_\text{2n} \\ \vdots & \vdots & \ddots & \vdots \\ a_\text{n1} & a_\text{n2} & \cdots & a_\text{nn} \end{pmatrix} \quad \text{det}(\mathbf{A}) = \sum_{\sigma \in S_\text{n}} \text{sgn}(\sigma) \prod_\text{i=1}^{n} a_{i, \sigma(i)}
$$

其中$$S_\text{n}$$为$$n$$的全排列集合，其中的元素为一组有序数组的各个排列方式。$$\text{sgn}(\sigma)$$为排列的符号，
...
$$
\text{det}(\mathbf{A}) = a_\text{11} (a_\text{22} a_\text{33} - a_\text{23} a_\text{32}) - a_\text{12} (a_\text{21} a_\text{33} - a_\text{23} a_\text{31}) + a_\text{13} (a_\text{21} a_\text{32} - a_\text{22} a\_\text{31})
$$

高阶行列式一般用拉普拉斯展开进行计算(用递推的方式降阶计算)

$$
\text{det}(\textbf{A}) = \sum_\text{i=1}^{n} (-1)^{i+j} a_\text{ij} \text{det}(\textbf{A}\_\text{ij})
$$
...
向量空间的基代表一种视角，为了简化问题，需要基变换和对应的坐标变换。同一个变换在不同视角下的表示便互为相似矩阵。设$$\{\mathbf{m_\text{1}}、\mathbf{m_\text{2}}、\mathbf{m_\text{3}}\cdots\mathbf{m_\text{s}}\}$$和$$\{\mathbf{n_\text{1}}、\mathbf{n_\text{2}}、\mathbf{n_\text{3}}\cdots\mathbf{n_\text{s}}\}$$是同一个向量空间的两个基，则存在唯一的矩阵$$\mathbf{P}$$，使得

$$
\begin{aligned}
(\mathbf{n_\text{1}}、\mathbf{n_\text{2}}、\mathbf{n_\text{3}}\cdots\mathbf{n_\text{s}})&=(\mathbf{m_\text{1}}、\mathbf{m_\text{2}}、\mathbf{m_\text{3}}\cdots\mathbf{m_\text{s}})\cdot \mathbf{P}\\
[x]_\text{N} &= \mathbf{P}^{-1}[x]_\text{M}
\end{aligned}
$$
...
&解得\lambda_\text{1} \approx 29.29, \quad \lambda_\text{2} \approx 4.71, \quad \lambda_\text{3} \approx 0.01\\
...
[MIT 线性代数](https://www.bilibili.com/video/BV16Z4y1U7oU/?spm_\text{i}d_\text{f}rom=333.337)

[3Blue1Brown 线性代数的本质](https://www.bilibili.com/video/BV1ys411472E/?spm_\text{i}d_\text{f}rom=333.999.0.0&vd_\text{s}ource=1d0891b41fe4e23dbf197eaf61dfa468)

[漫氏沉思录 无痛线代](https://www.bilibili.com/video/BV1wu411T7dj/?spm_\text{i}d_\text{f}rom=333.33)

[轩兔 矩阵论](https://www.bilibili.com/video/BV1PA411T7b5/?spm_\text{i}d_\text{f}rom=333.788&vd_\text{s}ource=1d0891b41fe4e23dbf197eaf61dfa468)

[Linear Algebra Done Right](https://linear.axler.net/)

[The Art of Linear Algebra](https://gitee.com/kin_\text{p}rofessor/The-Art-of-Linear-Algebra)
