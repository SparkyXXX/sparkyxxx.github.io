...
磁路是磁通量经过的闭合回路，通过主要路径的称主磁通$$\phi_\text{0}$$，此外的称漏磁通$$\phi_\text{s}$$。磁路的建立可以理解为，电流$$I$$产生磁场强度$$H$$，同时建立磁势$$F$$。 磁路分为气隙和铁芯两部分，其磁压降各有计算方式：气隙磁压降$$H_\text{0} = \frac{B_\text{0}}{\mu_\text{0}}$$，乘以气隙磁路的长度即可；铁芯磁压降算出或给出$$B$$，查表得到$$H$$，乘以铁芯磁路的长度即可。 一段磁路的磁阻在给定长度和截面积后可计算磁阻
...
空气的磁导率近似于真空磁导率，为常数$$\mu_\text{0}$$，由计算式$$R_\text{m} = \frac{L}{\mu S}$$ 可知气隙磁阻是线性的；铁磁性材料的磁导率不是常数，而是随外磁场的变化而变化，所以铁芯磁阻是非线性的。
...
给出$$\Phi = \Phi_\text{m}\sin\omega t$$，$$i_\text{0} = I_\text{0m}\sin\omega t$$后，感应电势可以计算得出

$$
\begin{aligned}
e_\text{1}&=-N_\text{1}\frac{\mathrm{d}\varPhi}{\mathrm{d}t}=\omega N_\text{1}\varPhi_\text{m}\sin(\omega t - 90^{\circ}) &\dot{E}_\text{1}&=-j\frac{2\pi fN_\text{1}\varPhi_\text{m}}{\sqrt{2}}=-j4.44fN_\text{1}\varPhi_\text{m}\\
e_\text{2}&=-N_\text{2}\frac{\mathrm{d}\varPhi}{\mathrm{d}t}=\omega N_\text{2}\varPhi_\text{m}\sin(\omega t - 90^{\circ}) &\dot{E}_\text{2}&=-j\frac{2\pi fN_\text{2}\varPhi_\text{m}}{\sqrt{2}}=-j4.44fN_\text{2}\varPhi_\text{m}\\
e_\text{1s}&=-\frac{\mathrm{d}\varPhi_\text{1s}}{\mathrm{d}t}=\omega L_\text{1s}I_\text{0m}\sin(\omega t - 90^{\circ}) &\dot{E}_\text{1s}&=-j\dot{I}_\text{0}2\pi fL_\text{1s}=-j\dot{I}_\text{0}X_\text{1s},\text{ 令}X_\text{1s}=\omega L_\text{1s}\\
e_\text{2s}&=-\frac{\mathrm{d}\varPhi_\text{2s}}{\mathrm{d}t}=\omega L_\text{2s}I_\text{0m}\sin(\omega t - 90^{\circ}) &\dot{E}_\text{2s}&=-j\dot{I}_\text{0}2\pi fL_\text{2s}=-j\dot{I}_\text{0}X_\text{2s},\text{ 令}X_\text{2s}=\omega L_\text{2s}
\end{aligned}
$$

对于输入电压$$u$$变化不大的情况，采用局部线性化的思想，认为$$\Phi_\text{m}$$有一个相对稳定的状态，对应$$L_\text{m}$$为一个常数，由$$\Psi = N\Phi = L_\text{m} i_\text{0}$$，代入感应电势表达式中得

$$
\begin{aligned}
e_\text{1} &= -\frac{\text{d}\Psi}{\text{d}t} = -L_\text{m}\frac{\text{d}i_\text{0}}{\text{d}t}\\
\dot{E_\text{1}} &= -j\omega L_\text{m} \dot{I_\text{0}} = -j\dot{I_\text{0}}X_\text{m}，令 X_\text{m} = \omega L_\text{m}\\
\end{aligned}
$$
...
输入电压$$\dot{U}$$产生空载电流$$\dot{I_\text{0}}$$(又称励磁电流)，该电流一方面产生空载磁势$$F_\text{0} = I_\text{0}N_\text{1}$$，另一方面产生损耗$$I_\text{0}r_\text{1}$$；也称无功分量$$\dot{I_\text{0a}}$$(用于发热)和有功分量$$\dot{I_{0\phi}}$$(铁芯损耗)。磁势$$F_\text{0}$$产生主磁通$$\Phi_\text{0}$$和漏磁通$$\Phi_\text{1s}$$，由于输入电压为交流电，磁通量也是变化的，变化的磁通量产生感应电动势，主磁通产生在一次回路和二次回路分别产生$$E_\text{1}$$和$$E_\text{2}$$，漏磁通在一次回路产生$$E_\text{1s}$$
...
代入$$\dot{E_\text{1s}} = -j\dot{I_\text{0}}X_\text{1s}$$和$$\dot{E_\text{1}} = -j\dot{I_\text{0}}X_\text{m}$$，可得
...
$$
\begin{aligned} &单极气隙内的磁通量&\Phi& = \int_\text{0}^\tau B(x)l\text{d}x\\ &极距(一个磁极在圆周上所占弧长)&\tau &= \frac{\pi D}{2p}，D 为直径，p 为极对数\\ &单极平均磁感应强度 &B_\text{cp}& = \frac{\Phi}{\tau l}，l 为绕组轴向长度，即电机圆柱体高度\\\ &每个有效边的感应电势 &e_\text{cp} &= B_\text{cp}lv = \frac{\Phi}{\tau}v\\ &电枢转速线速度(m/s) &v&= \frac{\pi D}{60}n，n 为 rpm 为单位的角速度\\ &总有效边数 &N& = 2SN_{y}，S 为绕组元件数，N_{y} 为每个元件含有的匝数 \end{aligned}
$$
...
$$
\begin{aligned} &单根导体受力大小&f& = Bi_\text{a} l,i_\text{a} 为支路电流，即每个导体中的电流\\ &单根导体产生力矩&m&= f\frac{D}{2}，D 为直径\\ &单根导体平均电磁力 &f_\text{cp}& = B_\text{cp}i_\text{a} l，l 为绕组轴向长度，即电机圆柱体高度\\ &单根导体电磁力矩 &m_\text{cp}& = f\_\text{cp}\frac{D}{2}\\ \end{aligned}
$$

总有效边数为$$N$$，支路数为$$2a$$，电枢绕组总电流$$I_\text{a} = 2a i_\text{a}$$。所有导体电磁转矩方向一致，总电磁转矩为

$$
 M_\text{em} = N\cdot f_\text{cp}\cdot\frac{D}{2} = N\cdot\frac{B\_\text{cp}I_\text{a}l}{2a}\cdot\frac{D}{2} = N\cdot\frac{\Phi}{\tau l}\cdot\frac{I_\text{a} l}{2a}\cdot\frac{D}{2} = \frac{Np}{2\pi a}\cdot\Phi I_\text{a} = C_\text{m}\Phi I_\text{a}
$$
...
工作过程：原动机提供机械转矩$$M_\text{1}$$，电枢绕组运动切割磁感线产生感应电势$$E_\text{a}$$。带载时在外电路形成电流回路，载流通电绕组在磁场中运动受力产生电磁阻转矩$$M_\text{em}$$，方向与原动转矩$$M_\text{1}$$相反。发电机空载指不带电负载，外电路开路。
...
- 转矩平衡，$$M_\text{1}$$为输入的原动转矩，$$M_\text{0}$$为空载转矩，$$M_\text{em}$$为电磁转矩(制动转矩)

  $$
  \begin{aligned}
  M_\text{1} &= M_\text{0} + M_\text{em}\\
  瞬态: M_\text{1} &= M_\text{0} + M_\text{em} + J\frac{\text{d}\Omega}{\text{d}t}\\
...
- 几个功率量如下，其中，两个$$P_\text{em}$$可以互相推导
  ...
  $$
  \begin{aligned}
  P_\text{0} &= M_\text{0}\Omega \quad 发电机空载的机械损耗\\
  P_\text{1} &= M_\text{1}\Omega \quad 原动机输入的机械功率\\
  P_\text{2} &= U_\text{a}I_\text{a} = I_\text{a}^2R_\text{L} \quad 发电机输出电功率，是最终输出功率\\
  P_\text{Cu} &= I_\text{a}^2R_\text{a} \quad 电枢绕组上的损耗，铜损\\
  P_\text{em} &= E_\text{a}I_\text{a} \quad 电路部分的输入功率\\
  同时，P_\text{em} &= M\_\text{em}\Omega \quad 原动机提供的把机械能转化为电能那部分的功率\\ \end{aligned}
  $$
...
- 功率平衡，由电势平衡和转矩平衡乘$$I_\text{a}$$可得(注：实际直流电机，有使通过电子调速器将直流变换成所谓的三相电作电子换向器以省略机械换向器，这时还会存在铁损，即磁滞损耗和涡流损耗)
  ...
  $$
  \begin{aligned}
  P_\text{em} &= P_\text{2} + P_\text{Cu}\\
  P_\text{1} &= P_\text{0} + P_\text{em} = P_\text{0} + P_\text{Cu}+ P_\text{2} \\
  \Sigma P &= P_\text{0} + P_\text{Cu} \quad 总损耗\\
  \eta &= \frac{P_\text{2}}{P_\text{1}}\times100\% = \frac{P_\text{2}}{P_\text{2} + \Sigma P} \times100\% \end{aligned}
  $$
...
直流发电机中$$P_\text{1} = M_\text{1} \Omega$$，$$M_\text{1}$$为原动机转矩，$$U_\text{a} = E_\text{a} - I_\text{a}R_\text{a}$$，$$E_\text{a} = C_\text{e} \Phi n$$为原动势，方向与$$U_\text{a} I_\text{a}$$相同，$$M_\text{1} = M_\text{0} + M_\text{em}$$，$$M_\text{em} = C_\text{m}\Phi I_\text{a}$$为阻转矩，电枢转向$$n$$与$$M_\text{1}$$相同，与$$M_\text{em}$$相反。
...
$$
\begin{aligned}
P_\text{1} &= P_\text{em} + P_\text{Cu}= P_\text{0} + P_\text{2} + P_\text{Cu}\\
P_\text{em} &= P_\text{0} + P_\text{2} \\
\Sigma P &= P_\text{0} + P_\text{Cu} \quad 总损耗\\
\eta &= \frac{P_\text{2}}{P_\text{1}}\times100\% = \frac{P_\text{2}}{P_\text{2} + \Sigma P} \times100\% \end{aligned}
$$
...
直流电动机中$$P_\text{1} = U_\text{a} I_\text{a}$$，$$U_\text{a}$$为电枢端电压；$$U_\text{a} = E_\text{a} + I_\text{a} R_\text{a}$$，$$E_\text{a} = C_\text{e} \Phi n$$为反电势，方向与$$U_\text{a} I_\text{a}$$相反，$$M_\text{em} = M_\text{0} + M_\text{2}$$，$$M_\text{em} = C_\text{m} \Phi I_\text{a}$$为驱动转矩，电枢转向$$n$$与$$M_\text{em}$$相同。

### 3.7 特性

#### 发电机

- 外特性：电机转速为额定转速，他励直流发电机的励磁电流达到额定值$$I_\text{jN}$$时，发电机负载两端电压$$U_\text{a}$$和负载电流$$I_\text{a}$$的关系。用电压调整率$$\Delta U_\text{a}$$来衡量发电机的恒定水平(对变负载情况的适应能力)，其中$$U_\text{a0}$$为空载输出电压，$$U_\text{aN}$$为额定运行时的输出电压$$ \Delta U*a = \frac{U*{a0} - U*{aN}}{U*{a0}} $$
...
- 机械特性(自然机械特性)：电枢电压$$U_\text{a}$$，磁通$$\Phi$$，电枢电阻$$R_\text{a}$$不变时，转速$$n$$与电磁转矩$$M_\text{em}$$之间的关系$$n = f(M)$$ n = \frac{U*\text{a}}{C*\text{e} \Phi} - \frac{R*\text{a}}{C*\text{e} C*\text{m} \Phi^2} M*\text{s} \\= n*\text{0} - K M*\text{s}
  ...
  稳态下的转矩记为$$M_\text{s}$$；$$M_\text{em} = 0$$时，$$n_\text{0} = \frac{U_\text{a}}{C_\text{e}\Phi}$$称为理想空载转速，实际空载总有$$M_\text{em} = M_\text{0}$$，因此实际空载转速小于理想空载转速。$$n = 0$$时，$$M_\text{d} = \frac{n_\text{0}}{K} = \frac{C_\text{m}\Phi U_\text{a}}{R_\text{a}}$$称为堵转转矩，是电机所能提供的最大转矩。对应的电枢电流称为堵转电流$$I_\text{d}$$，此时$$n = 0，E_\text{a} = 0$$，因此$$I_\text{d} = \frac{U_\text{a}}{R_\text{a}}$$；斜率$$K = \frac{\Delta n}{\Delta M}$$表示电机转速随转矩的变化程度
...
- 调节/控制特性：当$$M_\text{s}、R_\text{a}、\Phi$$不变时，转速$$n$$与电枢电压$$U_\text{a}$$的关系$$n = f(U_\text{a})$$。显然，调节关系是线性的
  ...
  \begin{aligned}
  n &= \frac{U_\text{a}}{C_\text{e} \Phi} - \frac{R_\text{a}}{C_\text{e}C_\text{m}\Phi^2}M_\text{s}\\
  n &= K_\text{1} U_\text{a} - K_\text{2} M_\text{s}
  \end{aligned}
  $$

  ![image-20250315223658077](../assets/post-pics/image-20250315223658077.png)

  当$$n = 0$$时，$$U_\text{a0} = \frac{K_\text{2}}{K_\text{1}} M_\text{s}$$称为起始电压；$$[0, U_\text{a0}]$$称为死区/不灵敏区，$$U_\text{a} > U_\text{a0}$$时才有转速输出；显然，负载转矩$$M_\text{s}$$增加时死区增大。

  调节斜率特性$$K_\text{1} = \frac{1}{C_\text{e} \Phi}$$，由电机结构决定与负载转矩无关。当电机以几到几十转每分运行时，由于齿槽、电刷换向器等造成电压不稳，摩擦力矩不均匀，电机出现时快时慢甚至停转的现象。
...
对于同步电机，定子侧和转子侧各有磁极，定子按照一定规律通两相或三相电流产生旋转磁场，磁场转速为$$n_\text{1}$$，定子和转子都产生磁场，两个磁场相互作用就会产生转矩。定子和转子之间的转速没有差异。
...
$$E_\text{2s}、X_\text{2s}、I_\text{2s}、\cos\Phi_\text{2s}$$是转子旋转时转子绕组的物理量，转子电流频率为$$f_\text{2} = sf_\text{1}$$
...
- 调速：$$n = (1 - s)n_\text{1} = (1 - s)\frac{60f_\text{1}}{p}$$，按照改变的物理量，调速分为两类

  - 同步转速$$n_\text{1}$$不变，改变转差率$$s$$，包括异步电机定子电压调速，绕线式异步电动机转子回路串电阻调速，绕线式异步电动机串级调速。

    - 改变定子电压调速，由于$$U_\text{1}$$只能从额定值往下调，故$$n$$一般也只往下调(转差率增大，转速下降)，适用于恒转矩负载，调速范围有限；调速时机械特性变软($$\frac{\Delta n}{\Delta M}$$增大)，且降低定子电压会使得过载能力下降($$\frac{M_\text{max}}{M_\text{N}}$$减小)

    ![image-20250315235040808](../assets/post-pics/image-20250315235040808.png)

    - 转子回路串电阻调速，$$r_\text{2}'$$增大，$$s_\text{m}$$增大，$$n$$减小，$$M_\text{max}$$不变；$$n$$只能往下调，$$r_\text{2}$$增加使功率损耗增加，效率降低
  ...
  - 改变旋转磁场同步转速$$n_\text{1}$$，包括改变定子极对数$$p$$调速，改变定子电源频率$$f_\text{1}$$调速。

    - 改变定子极对数调速即对于定子绕组采用不同的连接方式，可以改变旋转磁场的极对数，由于只能成对改变磁极，此调速方法是有级的。

    - 变频调速可以实现平滑且较大范围内的调速，关键是需要变频器。$$M_\text{max}$$反比于频率，$$f_\text{1}$$升高时$$M_\text{max}$$下降且机械特性变软($$\frac{\Delta n}{\Delta M}$$增大)，$$f_\text{1}$$降低时同理。 由$$U_\text{1}\approx E_\text{1} = 4.44f_\text{1} N_\text{1} k_\text{N1}\Phi_\text{m}$$，当$$U_\text{1}$$一定时，直接升高$$f_\text{1}$$会使得$$\Phi_\text{m}$$减小(铁芯利用率低)，转矩减小，同时频率增加使得转速增加；而降低$$f_\text{1}$$则会使得$$\Phi_\text{m}$$增大(铁芯容易过饱和且$$I_\text{1}$$增大)，转矩增大，同时频率降低使得转速降低。整个过程功率基本不变，适合恒功率调速。

      为了实现变频调速时磁通不变，可以相应改变$$U_\text{1}$$大小，使$$\frac{U_\text{1}}{f_\text{1}}$$不变，这种方式称为变频变压调速，可以保证磁通不变，转矩不变，适合于恒转矩负载。
...
功角在时间上是电网电压$$\dot{U_\text{1}}$$和励磁电压$$\dot{E_\text{0}}$$之间的相位差，在空间上是励磁磁场$$\dot{\Phi_\text{0}}$$和合成磁场$$\Sigma\Phi = \dot{\Phi_\text{0}} + \dot{\Phi_\text{a}} + \dot{\Phi_\text{s}}$$之间的空间夹角
...
当电网电压$$U_\text{1}$$和频率$$f_\text{1}$$不变，励磁电流$$I_\text{j}$$不变，$$\Phi_\text{0}，E_\text{0}$$不变时，同步电动机电磁功率和电磁转矩仅取决于功角大小。电磁功率$$P_\text{m}$$和功角$$\delta$$的关系$$P_\text{m} = f(\delta)$$称为同步电动机功角特性。电磁转矩$$M_\text{em}$$与功角$$\delta$$的关系$$M_\text{em} = f(\delta)$$称为同步电动机的矩角特性。
