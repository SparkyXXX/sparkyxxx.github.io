...
静态特性用多项式方程$$y = a_\text{0} + a_\text{1} x + a_\text{2} x^2 + \cdots + a_\text{n} x^n$$来表示，$$a_\text{0}$$称为零点输出，$$a_\text{1}$$称为理论灵敏度/线性灵敏度，其它项系数均为非线性系数。理想的传感器是线性的，输出和输入之间有严格的比例关系，因此会使用直线对实际的特性曲线进行拟合，线性度就是实际特性和拟合特性的差异，定义为：
...
- 均方误差(即假设各项误差互不影响)：线性度和迟滞属于系统误差、重复性属于偶然误差，理论上讲两者不能简单地直接叠加
  ...
  $$
  \delta = \pm \sqrt{\gamma_\text{L}^2 + \gamma_\text{H}^2 + \gamma_\text{R}^2 + \gamma_\text{S}^2}
  $$

- 求标准差(将拟合直线与实际曲线在各测点的误差看作随机分布)：用标准差表示
  $$
  \sigma = \sqrt{\frac{1}{n-1}\sum_\text{i=1}^n(\Delta y_\text{i})^2}
  $$
...
$$
R_\text{x} = \frac{x}{x_\text{max}}R_\text{max} \quad U_\text{x} = \frac{x}{x_\text{max}}U_\text{max}
$$
...
$$
k_\text{u} = \frac{\text{d}U_\text{x}}{\text{d}x} = \frac{2(b+h)\rho}{At}I \quad (电压灵敏度)
$$

![image-20250304121307146](../assets/post-pics/image-20250304121307146.png)

可改变节距$$t$$或设计成变骨架式电位器$$h = \frac{At}{2\rho I}\left( \frac{\text{d}U_\text{x}}{\text{d}x} \right) -b$$
...
$$
\begin{aligned}
&阶梯误差：:r_\text{j} = \pm \frac{\frac{1}{2}\cdot\frac{U_\text{max}}{N}}{U_\text{max}} = \pm\frac{1}{2N}\\
...
显然，根据$$R_\text{x}$$与$$R_\text{L}$$并联再与$$R_\text{max}-R_\text{x}$$串联的分压关系：

$$
\begin{aligned}
U_\text{o} &= \frac{\frac{R_\text{x}R_\text{L}}{R_\text{x} + R_\text{L}}}{\frac{R_\text{x}R_\text{L}}{R_\text{x} + R_\text{L}}+R_\text{max}-R_\text{x}}U\\
&= \frac{R_\text{x}R_\text{L}}{R_\text{L}R_\text{max}+R_\text{x}R_\text{max} - R_\text{x}^2}U\\
&=\frac{r}{1+rm(1-r)}U\\
其中r &= \frac{R_\text{x}}{R_\text{max}}(电阻相对变化)\\
...
负载输出$$U_\text{L} = \frac{r}{1+rm(1-r)}U_\text{max}$$，空载输出$$U_\text{0} = rU_\text{max}$$，将二者进行比较得到负载误差

$$
\begin{aligned}
\delta_\text{L} &= U_\text{0} - U_\text{L} \quad (绝对量)\\
\delta_\text{L} &= \frac{U_\text{0} - U_\text{L}}{U_\text{0}} = 1-\frac{U_\text{L}}{U_\text{0}} = 1-\frac{1}{1+rm(r-1)}
\end{aligned}
$$

由此可作出电位器的负载误差曲线，显然$$r=0.5$$时误差达到极大值，$$\delta_\text{L} = 1-\frac{1}{1+\frac{1}{4}m}$$，根据此式从要求的负载误差范围得出测量电路负载和电位器电阻值的关系，例如要求$$\delta_\text{L} < 0.01$$，则$$R_\text{L} > 25R_\text{max}$$
...
- 灵敏系数(应变片的灵敏系数受结构影响，会略小于应变片的灵敏系数，一般由实验测定)$$K = \frac{\frac{\Delta R}{R}}{\varepsilon} < K_\text{0}$$

- 横向效应：$$\Delta R = \Delta R_\text{x} - \Delta R_\text{y}$$

- 温度效应：电阻的变化来自应变和温度变化两部分，由温度变化引起的又细分为两部分，一是由电阻温度系数引起的，二是由应变片和试件膨胀系数不同造成附加形变引起的

  $$
  \begin{aligned}
  &\frac{\Delta R}{R} = \left(\frac{\Delta R}{R}\right)_\varepsilon + \left(\frac{\Delta R}{R}\right)_\text{t}\\
  ...
  &\left(\frac{\Delta R}{R}\right)_{t_\text{2}} = K\varepsilon' = K(\beta_\text{g} - \beta_\text{s})\Delta t \\
  &\left(\frac{\Delta R}{R}\right)_\text{t} = \left(\frac{\Delta R}{R}\right)_{t_\text{1}} + \left(\frac{\Delta R}{R}\right)_{t_\text{2}} = \alpha_\text{t} \Delta t + K(\beta_\text{g} - \beta_\text{s})\Delta t\\
  &\varepsilon_\text{t} = \frac{\left(\frac{\Delta R}{R}\right)_\text{t}}{K} = \frac{\alpha_\text{t} \Delta t}{K} + (\beta_\text{g} - \beta_\text{s})\Delta t \quad (虚假应变)\\
  &e_\text{t} = \frac{\varepsilon_\text{t}}{\varepsilon} \times 100 \% \quad (温度误差)
  \end{aligned}
  $$
...
$$
\begin{aligned}
&U_\text{o}=\frac{R_\text{1}}{R_\text{1} + R_\text{2}}U-\frac{R_\text{3}}{R_\text{3} + R_\text{4}}U=\frac{R_\text{1}R_\text{4}-R_\text{2}R_\text{3}}{(R_\text{1}+R_\text{2})(R_\text{3}+R_\text{4})}U\\
&故R_\text{1}R_\text{4}=R_\text{2}R_\text{3} \quad 或 \quad \frac{R_\text{1}}{R_\text{2}}=\frac{R_\text{3}}{R_\text{4}}
\end{aligned}
$$

定义桥臂比$$n = \frac{R_\text{2}}{R_\text{1}}$$，单臂电桥的电压灵敏度($$R_\text{1}$$变化为$$R_\text{1} + \Delta R_\text{1}$$代入式子)：

$$
\begin{aligned}
输出电压:U_\text{o}&=\frac{(R_\text{1}+\Delta R_\text{1})R_\text{4}-R_\text{2}R_\text{3}}{(R_\text{1}+\Delta R_\text{1}+R_\text{2})(R_\text{3}+R_\text{4})}U\\
&= \frac{\frac{R_\text{4}}{R_\text{3}}\frac{\Delta R_\text{1}}{R_\text{1}}}{(1 + \frac{R_\text{2}}{R_\text{1}}+\frac{\Delta R_\text{1}}{R_\text{1}})(1+\frac{R_\text{4}}{R_\text{3}})}U\\
&\approx\frac{n}{(1 + n)^{2}}\frac{\Delta R_\text{1}}{R_\text{1}}U\\
电压灵敏度:K_\text{u}&=\frac{U_\text{0}}{\Delta R_\text{1}/R_\text{1}}=\frac{n}{(1 + n)^{2}}U
\end{aligned}
$$
...
$$
\begin{aligned}
&理想输出 \quad U_\text{o}'=\frac{1}{4}\frac{\Delta R_\text{1}}{R_\text{1}}U\\
&实际输出 \quad U_\text{o}=\frac{\frac{\Delta R_\text{1}}{R_\text{1}}}{4(1+\frac{1}{2}\frac{\Delta R_\text{1}}{R_\text{1}})}U\\
&非线性误差 \quad \delta=\frac{U_\text{o}-U_\text{o}'}{U_\text{o}'}=-\frac{1}{2}\frac{\Delta R_\text{1}}{R_\text{1}}
\end{aligned}
$$
...
上述模式被称为工作在小偏差不平衡电桥的工作模式，与不平衡电桥相对应的还有平衡电桥，其中$$R_\text{1}$$为应变片电阻，$$R_\text{2}$$为可调电阻，调节$$R_\text{2}$$使得电桥平衡(检流计读数为 0)。这种方式由于需要人为将电桥调平衡，故只适用于静态测量

$$
\frac{R_\text{1} + \Delta R_\text{1}}{R_\text{2} + \Delta R_\text{2}} = \frac{R_\text{3}}{R_\text{4}}
$$
...
单丝补偿法即通过调节参数匹配，使$$\alpha_\text{t} = -k(\beta_\text{g} - \beta_\text{s})$$。控制合金成分和退火温度可以控制$$\alpha_\text{t}$$，这种方法的温度适用范围有限，且材料通用性差。组合式自补偿法即串接不同金属丝，使得$$\Delta R_\text{1} = -\Delta R_\text{2}$$，只需$$\frac{\alpha_\text{1}}{\alpha_\text{2}} = -\frac{R_\text{2}}{R_\text{1}}$$即可。另一种从外部电路进行补偿，由于电阻变化值非常小，将应变片作为电阻接入电桥，将应变转化为电学量进行测量，下面详细介绍。

四个电桥电阻均会受到温度影响

$$
\begin{aligned}
U_\text{o}&=\frac{\frac{R_\text{2}}{R_\text{1}}}{(1 + \frac{R_\text{2}}{R_\text{1}})^2}\left(\frac{\Delta R_\text{1}}{R_\text{1}}-\frac{\Delta R_\text{2}}{R_\text{2}}+\frac{\Delta R_\text{4}}{R_\text{4}}-\frac{\Delta R_\text{3}}{R_\text{3}}\right)(1 - \eta)U\\
\eta&=\frac{1}{1+\frac{(1 + \frac{R_\text{2}}{R_\text{1}})}{\frac{\Delta R_\text{1}}{R_\text{1}}+\frac{\Delta R_\text{3}}{R_\text{3}}+\frac{R_\text{2}}{R_\text{1}}\left(\frac{\Delta R_\text{2}}{R_\text{2}}+\frac{\Delta R_\text{4}}{R_\text{4}}\right)}}U \\
&\frac{\Delta R_\text{i}}{R_\text{i}}\ll 1时，\eta = 0\\
U_\text{o}&=\frac{\frac{R_\text{2}}{R_\text{1}}}{(1 + \frac{R_\text{2}}{R_\text{1}})^2}\left(\frac{\Delta R_\text{1}}{R_\text{1}}-\frac{\Delta R_\text{2}}{R_\text{2}}+\frac{\Delta R_\text{4}}{R_\text{4}}-\frac{\Delta R_\text{3}}{R_\text{3}}\right)U\\
&=\frac{n}{(1 + n)^2}\left(\frac{\Delta R_\text{1}}{R_\text{1}}-\frac{\Delta R_\text{2}}{R_\text{2}}+\frac{\Delta R_\text{4}}{R_\text{4}}-\frac{\Delta R_\text{3}}{R_\text{3}}\right)U\\
对于等臂电桥:U_\text{o}
&=\frac{1}{4}\left(\frac{\Delta R_\text{1}}{R_\text{1}}-\frac{\Delta R_\text{2}}{R_\text{2}}+\frac{\Delta R_\text{4}}{R_\text{4}}-\frac{\Delta R_\text{3}}{R_\text{3}}\right)U
\end{aligned}
$$
...
$$
\begin{aligned}
\frac{\Delta R_\text{1}}{R_\text{1}}&=\left(\frac{\Delta R_\text{1}}{R_\text{1}}\right)_{\varepsilon}+\left(\frac{\Delta R_\text{1}}{R_\text{1}}\right)_\text{t}\\
\frac{\Delta R_\text{2}}{R_\text{2}}&= 0+\left(\frac{\Delta R_\text{2}}{R_\text{2}}\right)_\text{t}\\
U_\text{o}&=\frac{1}{4}\left(\left(\frac{\Delta R_\text{1}}{R_\text{1}}\right)_{\varepsilon}+\left(\frac{\Delta R_\text{1}}{R_\text{1}}\right)_\text{t}-\left(\frac{\Delta R_\text{2}}{R_\text{2}}\right)_\text{t}+\frac{\Delta R_\text{4}}{R_\text{4}}-\frac{\Delta R_\text{3}}{R_\text{3}}\right)U\\
...
\begin{aligned}
\frac{\Delta R_\text{1}}{R_\text{1}}&=\left(\frac{\Delta R_\text{1}}{R_\text{1}}\right)_{\varepsilon}+\left(\frac{\Delta R_\text{1}}{R_\text{1}}\right)_\text{t}\\
\frac{\Delta R_\text{2}}{R_\text{2}}&=\left(\frac{\Delta R_\text{2}}{R_\text{2}}\right)_{-\varepsilon}+\left(\frac{\Delta R_\text{1}}{R_\text{1}}\right)_\text{t}\\
U_\text{o}&=\frac{1}{4}\left(\left(\frac{\Delta R_\text{1}}{R_\text{1}}\right)_{\varepsilon}-\left(\frac{\Delta R_\text{2}}{R_\text{2}}\right)_{-\varepsilon}+\frac{\Delta R_\text{4}}{R_\text{4}}-\frac{\Delta R_\text{3}}{R_\text{3}}\right)U\\
...
$$
U_\text{0}=\frac{\frac{R_\text{3}}{R_\text{2}}}{\frac{R_\text{3}}{R_\text{2}}+1}(1 + \frac{R_\text{4}}{R_\text{1}})U_\text{i2}-\frac{R_\text{4}}{R_\text{1}}U_\text{i1}
$$
...
$$
\begin{aligned}
U_\text{0}&=(1 + \frac{R+\Delta R}{R})\frac{R}{R + R}U_\text{i}-\frac{R+\Delta R}{R}U_\text{i}\\
U_\text{0}&=-\frac{\Delta R}{2R}U_\text{i}=-\frac{1}{2}K\varepsilon U_\text{i}
\end{aligned}
$$
...
电感值与线圈匝数，线圈形状，导磁材料等都有关。基本原理为：衔铁摆动使得气隙磁阻变化，引起电感值的改变。以典型的单边自感式传感器为例，电感值与结构参数的关系如下，其中$$W$$为线圈匝数，$$\mu_\text{0} = 4\pi \times 10^{-7} H/m$$为真空导磁率，$$S_\text{0}$$为空气隙有效截面积，$$l_\text{0}$$为空气隙长度。

$$
\begin{aligned}
\Phi &= \frac{WI}{R_\text{m}} \quad (\frac{磁动势}{磁阻})\\
R_\text{m} &= \sum_\text{i=1}^n\frac{l_\text{i}}{\mu_\text{i} S_\text{i}} + 2\frac{l_\text{o}}{\mu_\text{o} S_\text{o}} \quad (\mu_\text{i} \gg \mu_\text{o})\\
L &= \frac{\Psi}{I} = \frac{W\Phi}{I} = \frac{W^2}{R_\text{m}} = \frac{W^{2}\mu_\text{0}S_\text{0}}{2l_\text{0}}
\end{aligned}
$$
...
- 灵敏度：
  ...
  $$
  \begin{aligned}
  L&=\frac{A}{l} \quad A = \frac{W^2 \mu_\text{0} S_\text{0}}{2}\\
  \Delta L&=L - L_\text{0}=\frac{A}{l_\text{0}+\Delta l}-\frac{A}{l_\text{0}}\\
  &=\frac{A}{l_\text{0}}\left(\frac{1}{1+\frac{\Delta l}{l_\text{0}}}-1\right)=L_\text{0}\left(\frac{1}{1+\frac{\Delta l}{l_\text{0}}}-1\right)\\
  \Delta L&=-L_\text{0}\frac{\Delta l}{l_\text{0}}+L_\text{0}\left(\frac{\Delta l}{l_\text{0}}\right)^{2}\frac{1}{1+\frac{\Delta l}{l_\text{0}}} \approx -L_\text{0}\frac{\Delta l}{l_\text{0}}\\
  即&: 灵敏度K = -\frac{L_\text{0}}{l_\text{0}}\\
  \end{aligned}
  $$

  因此，为了提高灵敏度和减小非线性误差，$$l_\text{0}$$和$$\Delta l$$都不能过大。同时，从式子可见，变间隙式传感器的灵敏度和线性度是矛盾的。

  对于差动式，分析如下：

  $$
  \begin{aligned}
  \Delta L_\text{1}&=-L_\text{0}\frac{\Delta l}{l_\text{0}}+L_\text{0}\left[\left(\frac{\Delta l}{l_\text{0}}\right)^2 - \left(\frac{\Delta l}{l_\text{0}}\right)^3 + \left(\frac{\Delta l}{l_\text{0}}\right)^4-\cdots\right]\\
  \Delta L_\text{2}&=L_\text{0}\frac{\Delta l}{l_\text{0}}+L_\text{0}\left[\left(\frac{\Delta l}{l_\text{0}}\right)^2 + \left(\frac{\Delta l}{l_\text{0}}\right)^3 + \left(\frac{\Delta l}{l_\text{0}}\right)^4+\cdots\right]\\
  \Delta L&=\Delta L_\text{1} - \Delta L_\text{2}=-2L_\text{0}\frac{\Delta l}{l_\text{0}}-2L_\text{0}\left(\frac{\Delta l}{l_\text{0}}\right)^3\left[1 + \left(\frac{\Delta l}{l_\text{0}}\right)^2 + \left(\frac{\Delta l}{l_\text{0}}\right)^4+\cdots\right]\\
  \end{aligned}
  $$
...
- 调幅电路：主要有电桥式和谐振式两种
  ...
  1. 电阻平衡臂电桥($$Z_\text{1}$$为固定电感，$$Z_\text{2}$$变为$$Z_\text{2} + \Delta Z$$)
     $$
     \begin{aligned}
     Z&=r + j\omega L\\
     ...
     \dot{U}_\text{o}&=\left(\frac{Z_\text{2}}{Z_\text{1} + Z_\text{2}}-\frac{R}{R + R}\right)\dot{U}\\
  ...
  $$
  \begin{aligned}
  \dot{U}_\text{o}&=\frac{Z_\text{1}}{Z_\text{1} + Z_\text{2}}\dot{U}-\frac{1}{2}\dot{U}\\
...
- 调相电路
  ...
  \dot{U}_\text{o}&=\frac{R}{R + j\omega L}\dot{U}-\frac{1}{2}\dot{U}\\
...
$$
\begin{aligned}
\dot{E}_\text{2a}&=-j\omega M_\text{a} \dot{I}_\text{1}\quad\dot{E}_\text{2b}=-j\omega M_\text{b} \dot{I}_\text{1}\\
M_\text{a}&=\frac{W_\text{1a}W_\text{2a}}{R_\text{ma}}\quad M_\text{b}=\frac{W_\text{1b}W_\text{2b}}{R_\text{mb}}\\
Q&=\frac{\omega L}{r}\\
\text{假设：}W_\text{1a}&=W_\text{2a}=W_\text{1},\ r_\text{1a}=r_\text{1b}=r_\text{1}\\
W_\text{1b}&=W_\text{2b}=W_\text{2},\ L_\text{1a0}=L_\text{1b0}=L_\text{10}\\
\dot{E}_\text{2}&=\dot{E}_\text{2a}-\dot{E}_\text{2b}=-j\omega\dot{I}_\text{1}(M_\text{a} - M_\text{b})\\
&=\dot{U}_\text{1}\cdot\frac{W_\text{2}}{W_\text{1}}\cdot\frac{\Delta l}{l_\text{0}}\cdot\frac{1 + j\frac{1}{Q}}{1+\frac{1}{Q^{2}}}\\
\vert \dot{E_\text{2}} \vert &=U_\text{1}\frac{W_\text{2}}{W_\text{1}}\frac{\Delta l}{l_\text{0}}\frac{1}{\sqrt{1+\frac{1}{Q^{2}}}}
\end{aligned}
$$

$$
\begin{cases}
\text{当}\omega L_\text{10}\ll r_\text{1}\text{时，}Q\ll1,\text{灵敏度}K = U_\text{1}\frac{W_\text{2}}{W_\text{1}}\frac{1}{l_\text{0}}\frac{\omega L_\text{10}}{r_\text{1}}\\
\text{当}\omega L_\text{10}\gg r_\text{1}\text{时，}Q\gg1,\text{灵敏度}K = U_\text{1}\frac{W_\text{2}}{W_\text{1}}\frac{1}{l_\text{0}}\\
\end{cases}
$$
...
- 增加$$\frac{W2}{W1}$$ 和减小$$l_\text{0}$$都能使灵敏度 K 值提高。($$\frac{W2}{W1}$$影响体积及零点残余电压，一般$$l_\text{0}$$为 0.5mm)
...
测量电路需要解决的，就是位移方向和大小的测量，因此需要鉴别相位，同时需要消除零点残余电压，一般采用差动整流电路：$$\dot{U}_\text{0}=\dot{U}_\text{24}-\dot{U}_\text{68} $$，输出的$$U\_\text{o}$$为双极性电压，通过极性就可以判断位移方向。
...
线圈的交变电流产生磁场$$\Phi_\text{1}$$，磁场$$\Phi_\text{1}$$在金属体内产生感生电流/涡流以阻碍$$\Phi_\text{1}$$的变化(交流电线圈和金属体之间的互感效应)，使得金属体的阻抗发生改变。金属导体作为被测对象，可以利用该性质进行测量。
...
$$
\begin{aligned}
&R_\text{1}\dot{I}_\text{1} + j\omega L_\text{1}\dot{I}_\text{1} - j\omega M\dot{I}_\text{2} = \dot{U} \\
&R_\text{2}\dot{I}_\text{2} + j\omega L_\text{2}\dot{I}_\text{2} - j\omega M\dot{I}_\text{1} = 0 \\
&Z = R_\text{1} + R_\text{2}\frac{\omega^2 M^2}{R_\text{2}^2 + \omega^2 L_\text{2}^2}+j\omega\left(L_\text{1} - L_\text{2}\frac{\omega^2 M^2}{R_\text{2}^2 + \omega^2 L_\text{2}^2}\right)
\end{aligned}
$$

其中，$$R_\text{1} + R_\text{2}\frac{\omega^2 M^2}{R_\text{2}^2 + \omega^2 L_\text{2}^2}$$称为等效电阻，$$R_\text{2}\frac{\omega^2 M^2}{R_\text{2}^2 + \omega^2 L_\text{2}^2}$$称为反射电阻；$$L_\text{1} - L_\text{2}\frac{\omega^2 M^2}{R_\text{2}^2 + \omega^2 L_\text{2}^2}$$称为等效电感，$$L_\text{2}\frac{\omega^2 M^2}{R_\text{2}^2 + \omega^2 L_\text{2}^2}$$称为反射电感。对于非磁性材料而言，当金属导体和线圈靠近时，涡流效应使得等效电阻增加，有功损耗增大，同时等效电感减小，无功损耗减小，表现为去磁作用；对于磁性材料而言，靠近过程中自感系数$$L_\text{1}$$会显著增大，表现为增磁作用。
...
- 调频电路：谐振调幅电路，阻抗变化使得线圈阻值变化，并通过谐振频率的偏移反应在输出信号的幅值上。采用并联谐振的方式，谐振时阻抗最高，对应输出电压最高。当电路谐振频率不等于石英晶振振荡源提供的频率$$f_\text{0}$$(也称初始谐振频率)时，称为失谐。

  ![image-20250319103432408](../assets/post-pics/image-20250319103432408.png)

  对于磁性材料，随着线圈与金属体的靠近，自感系数升高，谐振频率降低，阻抗降低，输出幅值下降；对于非磁性材料，随着线圈与金属体的靠近，自感系数降低，谐振频率增加，阻抗降低，输出幅值下降。(在$$f = f_\text{0}$$处读取输出信号幅值)
...
- 利用一个方向磁导率的改变：受力使得磁导率改变，引起互感变化，阻抗变化，使得输出电压发生变化
  ...
  $$
  \begin{aligned}
  L &= K_\text{1}u = K_\text{2}P\\
  E_\text{2} &= \frac{W_\text{1}}{W_\text{2}}K(P)U_\text{1}P\\
  \end{aligned}
  $$
...
$$
\begin{aligned}
C_\text{0}&=\frac{\varepsilon S}{d_\text{0}}\\
C&=\frac{\varepsilon S}{d}\\
C&=\frac{\varepsilon S}{d_\text{0}+\Delta d}=C_\text{0}+\Delta C\\
\Delta C&=-\frac{C_\text{0}}{d_\text{0}}\Delta d + C_\text{0}\left(\frac{\Delta d}{d_\text{0}}\right)^2\left[1 - \left(\frac{\Delta d}{d_\text{0}}\right) + \left(\frac{\Delta d}{d_\text{0}}\right)^2\cdots\right]\\
k&=\frac{\Delta C}{\Delta d}=-\frac{C_\text{0}}{d_\text{0}}\left[1 - \left(\frac{\Delta d}{d_\text{0}}\right) + \left(\frac{\Delta d}{d_\text{0}}\right)^2\cdots\right]
\end{aligned}
$$

可见，灵敏度规律与自感式传感器类似，线性度和灵敏度对传感器的结构要求是矛盾的：只有比值$$\frac{\Delta d}{d_\text{0}}$$很小时才可认为是接近线性关系。可适当增大初始距离$$d_\text{0}$$，以保证比值$$\frac{\Delta d}{d_\text{0}}$$不致过大；但灵敏度下降，电容传感器初值减小。常采用差动式电容传感器。

#### 差动式电容传感器

$$
\begin{aligned}
C_\text{1}&=C_\text{0}\frac{1}{1 - \frac{\Delta d}{d_\text{0}}} \quad C_\text{2} = C_\text{0}\frac{1}{1 + \frac{\Delta d}{d_\text{0}}}\\
C_\text{1}&=C_\text{0}\left[1 + \left(\frac{\Delta d}{d_\text{0}}\right) + \left(\frac{\Delta d}{d_\text{0}}\right)^2 + \left(\frac{\Delta d}{d_\text{0}}\right)^3 + \cdots\right]\\
C_\text{2}&=C_\text{0}\left[1 - \left(\frac{\Delta d}{d_\text{0}}\right) + \left(\frac{\Delta d}{d_\text{0}}\right)^2 - \left(\frac{\Delta d}{d_\text{0}}\right)^3 + \cdots\right]\\
\Delta C&=C_\text{1} - C_\text{2} = C_\text{0}\left[2\left(\frac{\Delta d}{d_\text{0}}\right) + 2\left(\frac{\Delta d}{d_\text{0}}\right)^3 + 2\left(\frac{\Delta d}{d_\text{0}}\right)^5 + \cdots\right]\\
\frac{\Delta C}{C_\text{0}}&=2\frac{\Delta d}{d_\text{0}}\left[1 + \left(\frac{\Delta d}{d_\text{0}}\right)^2 + \left(\frac{\Delta d}{d_\text{0}}\right)^4 + \left(\frac{\Delta d}{d_\text{0}}\right)^6 + \cdots\right]
\end{aligned}
$$

与自感式传感器类似，差动式结构可以同时改善灵敏度和线性度

#### 变面积型电容器

![image-20250325002333661](../assets/post-pics/image-20250325002333661.png)

平板型电容器：$$C=\frac{\varepsilon_\text{0}\varepsilon_\text{r} (a - \Delta x)b}{d}$$，$$\frac{\Delta C}{C_\text{0}}=\frac{\Delta x}{a}$$

圆柱形电容器：$$C = \frac{2\pi\varepsilon_\text{0}\varepsilon_\text{r} l}{\ln\frac{R}{r}}$$
...
$$
\begin{aligned}
C&=\frac{C_\text{1}\cdot C_\text{2}}{C_\text{1} + C_\text{2}}=\frac{\varepsilon_\text{0}\varepsilon_\text{1} A}{\varepsilon_\text{1} d_\text{0} + d_\text{1}}\\
C_\text{0}&=\frac{\varepsilon_\text{0} A}{d_\text{0} + d_\text{1}}\\
\Delta C&=C - C_\text{0} = C_\text{0}\cdot\frac{\varepsilon_\text{1} - 1}{\varepsilon_\text{1}\frac{d_\text{0}}{d_\text{1}}+1}
\end{aligned}
$$
...
$$
\begin{aligned}
C&=C_\text{1} + C_\text{2}=\frac{\varepsilon_\text{0}\varepsilon_\text{1} A_\text{1} + \varepsilon_\text{0} A_\text{2}}{d}\\
C_\text{0}&=\frac{\varepsilon_\text{0}(A_\text{1} + A_\text{2})}{d}\\
\Delta C&=C - C_\text{0}=\frac{\varepsilon_\text{0} A_\text{1}(\varepsilon_\text{1} - 1)}{d}
\end{aligned}
$$
...
$$
\begin{aligned}
C_\text{0}&=\frac{2\pi\varepsilon_\text{0} H}{\ln\frac{D}{d}}\\
C&=C_\text{1} + C_\text{2}\\
&=\frac{2\pi\varepsilon_\text{0} (H - h)}{\ln\frac{D}{d}}+\frac{2\pi\varepsilon_\text{0}\varepsilon_\text{1} h}{\ln\frac{D}{d}}\\
&=\frac{2\pi\varepsilon_\text{0} H}{\ln\frac{D}{d}}+\frac{2\pi\varepsilon_\text{0} h(\varepsilon_\text{1} - 1)}{\ln\frac{D}{d}}\\
\Delta C&=C - C_\text{0}=\frac{2\pi\varepsilon_\text{0} h(\varepsilon_\text{1} - 1)}{\ln\frac{D}{d}}
\end{aligned}
$$
...
$$
\begin{aligned}
\frac{1}{j\omega C_\text{e}}&=j\omega L+\frac{1}{j\omega C}\\
...
- 电桥测量电路
  ...
  $$
  \begin{aligned}
  \frac{\frac{R_\text{2}}{j\omega C_\text{2}}}{R_\text{2}+\frac{1}{j\omega C_\text{2}}}R_\text{3}&=\frac{\frac{R_\text{1}}{j\omega C_\text{1}}}{R_\text{1}+\frac{1}{j\omega C_\text{1}}}R_\text{4}\\
  R_\text{2}R_\text{3}+j\omega R_\text{1}R_\text{2}R_\text{3}C_\text{1}&=R_\text{1}R_\text{4}+j\omega R_\text{1}R_\text{2}R_\text{4}C_\text{2}\\
  实部:
  R_\text{2}&=\frac{R_\text{4}}{R_\text{3}}R_\text{1}\\
  虚部:
  C_\text{2}&=\frac{R_\text{4}}{R_\text{3}}C_\text{1}
  \end{aligned}
  $$
...
- 双 T 型充放电网络
  ...
  $$
  \begin{aligned}
  \text{时间常数}\tau&=(R+\frac{R_\text{L}R}{R_\text{L}+R})C_\text{2}\\
  i_\text{C2}&=\left(\frac{U+\frac{R_\text{L}}{R_\text{L}+R}U}{R+\frac{RR_\text{L}}{R_\text{L}+R}}\right)\exp^{\frac{-t}{(R+\frac{R_\text{L}R}{R_\text{L}+R})C_\text{2}}}\\
  I_\text{C2}&=\frac{1}{T}\int_\text{0}^{\frac{T}{2}}i_\text{C2}dt\approx\frac{1}{T}\int_\text{0}^{\infty}i_\text{C2}dt=\frac{1}{T}\frac{R + 2R_\text{L}}{R+R_\text{L}}UC_\text{2}\\
  I_\text{C1}&=\frac{1}{T}\frac{R + 2R_\text{L}}{R+R_\text{L}}UC_\text{1}\\
  U_\text{0}&=\frac{RR_\text{L}}{R + R_\text{L}}(I_\text{C1}-I_\text{C2})=\frac{RR_\text{L}(R + 2R_\text{L})}{(R+R_\text{L})^{2}}\frac{U}{T}(C_\text{1}-C_\text{2})\\
  &=K\cdot U\cdot f\cdot(C_\text{1}-C_\text{2})
  \end{aligned}
  $$
...
- 差动脉冲调宽型电路
  ...
  $$
  \begin{aligned}
  U_\text{c}/U_\text{d}&=U(1 - e^{-\frac{t}{\tau_\text{1}}})\\
  T_\text{1}&=R_\text{1}C_\text{1}\ln\frac{U}{U - E}\\
  T_\text{2}&=R_\text{2}C_\text{2}\ln\frac{U}{U - E}\\
  u_\text{0}&=(u_\text{AB})_{\text{DC}} = u_\text{A}-u_\text{B}\\
  &=\frac{T_\text{1}}{T_\text{1}+T_\text{2}}u_{\text{Am}}-\frac{T_\text{2}}{T_\text{1}+T_\text{2}}u_{\text{Bm}}\\
  &=\frac{C_\text{1}-C_\text{2}}{C_\text{1}+C_\text{2}}u_\text{m}\\
  \text{变极距型: }U_\text{0}&=\frac{d_\text{2}-d_\text{1}}{d_\text{2}+d_\text{1}}U \to u_\text{0}=\pm\frac{\Delta d}{d_\text{0}}u_\text{m}\\
  \text{变面积型: }u_\text{0}&=\pm\frac{\Delta A}{A_\text{0}}u_\text{m}
  \end{aligned}
  $$
